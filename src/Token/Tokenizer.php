...
class Tokenizer implements TokenizerInterface {
  ...
  protected function process(array $patterns, string $input, array $refinements, int &$line, int &$column, TokenStream $stream): void {
    ...
    while (strlen($input)) {
      ...
      if (isset($this->refinements[$token]) && !in_array($token, $refinements)) {
        ...
        $stream->addToken($this->createToken($token, $value, $line, $column));
  ...
  /**
   * Creates a token for the stream.
   *
   * @param string $type
   *   The type.
   * @param string $value
   ...
   *   The token.
   */
  protected function createToken(string $type, string $value, int $line, int $column): TokenInterface {
